...
After starting the server, run the demo script in a second terminal:

```bash
./demo_requests.sh
```

The appeal command uses PASTE_CONTENT_ID_HERE as a placeholder. Replace it with a real content_id from a submit response.

## Local Testing Checklist

1. Start the server with python app.py.
2. Submit one likely AI text with POST /submit.
3. Submit one casual human-like text with POST /submit.
4. Call GET /log and confirm at least two entries appear.
5. Submit an appeal for one content_id with POST /appeal.
6. Call GET /log again and confirm the appealed entry has status under_review.
7. Rate limiting is configured on POST /submit at 10 per minute and 100 per day.
